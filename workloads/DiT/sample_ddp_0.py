...
def main(args):
    ...
    total_labels = torch.arange(args.num_classes, device=device).repeat_interleave(total_samples // args.num_classes)
    # Split among GPUs
    samples_needed_this_gpu = total_samples // dist.get_world_size()
    start = rank * samples_needed_this_gpu
    end = (rank + 1) * samples_needed_this_gpu
    labels_this_gpu = total_labels[start:end].to(device)
    # print(f"labels_this_gpu: {labels_this_gpu}")
